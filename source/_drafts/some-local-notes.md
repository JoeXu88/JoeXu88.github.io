...
#### 数据结构与算法  
* [算法归总，面试](https://wizardforcel.gitbooks.io/the-art-of-programming-by-july/content/00.01.html)  
...
* 二分查找无处不在，但是二分也有它的局限性：首先只能查找有序序列，也就是根据排好序的数据来缩小范围进行查找；局限于数组类型，如果是链表就会退化到O(n)。二分查找的临界点是包含等于的，也就是说可以查找等于某个值的，或者大于等于，小于等于的值，而不能单独查找说大于某个值的，因为范围不好缩小到最后，找不到退出的临界点。假设我们要查找第一个大于某个值的点，可以转换为查找最后一个小于等于的点，然后下一个就是第一个大于的，同理要查找最后一个小于某个值的点，就转换为查找第一个大于等于的点。那如果查找第一个小于等于的，或者最后一个大于等于呢，这个其实没有意义，因为直接查看第一个值或者最后一个值就行，没有查找的必要。不能用于数据量太大或者太小，到底多大是打多小是小需要再斟酌。 
* redis 是用跳表实现的，也就是k级索引，可以利用空间来降低时间复杂度到logn 以优化单纯链表的实现。Redis 不用红黑树是因为redis 要支持区间查找，这个效率跳表要比较高。跳表的实现也要比红黑树简单很多。   
* hash 表(散列表或者哈希表)其实就是key & value 表，原理来自于数组，也就是可以随机访问成员。通过存储key 和 value 的对应，然后借由 key 值来获取 value。实例中Word的拼写检查就是将字典所有词存在内存的hash 表中，通过检查单次是否存在在hash 表中来判断是否拼写正确。hash 表的设计核心是 hash 函数和冲突的解决。  
* A % B = A & (B - 1)  
* hash 表经常与链表或者跳表结合使用，hash表用来快速定位节点，链表用来顺序遍历节点或者有序排列节点。  
* 哈希算法可以用于数据加密，数据校验，唯一标识，hash 表等。但是哈希算法并不能保证绝对的唯一，因为一般哈希的结果长度是固定的，所以可以枚举的数量是固定的，只能说长度越长重复的概率越小。  
* [一致性哈希算法参考](https://yikun.github.io/2016/06/09/%E4%B8%80%E8%87%B4%E6%80%A7%E5%93%88%E5%B8%8C%E7%AE%97%E6%B3%95%E7%9A%84%E7%90%86%E8%A7%A3%E4%B8%8E%E5%AE%9E%E8%B7%B5/)  
...
* 快排可以用于寻找数组的第 K 大元素，也就是分区的理念引申而来。假设分区到位置p， 那么p 位置左边都比a[p] 小，所以如果 p+1==K， 那么第[p+1] 个就是第 K 大的元素。
* 大文件的排序
1> 每个文件300M，总共10个文件，每个文件内部是时间顺序排列。用尽量快的速度结合所有文件到一个文件中。
解法思路1： 从每个文件中读取一行到内存中，建立最小堆，然后取堆顶写入文件，接着从选取的最小时间对应的文件中继续读取一行插入最小堆，然后继续取堆顶元素写入文件，重复以上操作直到没有文件内容。
解决思路2： 分配10个数组，每个数组40M，再分配400M的数组。然后自己每个文件批量读取40M 左右，分配好到10个数组中，然后取10个数组最大的时间戳的最小值，然后利用二分搜索在每个数组中找到相应位置，并将该位置之前的数据都拷贝到400M内存中，最后将这个400M的数组利用快排后一次性写入结果文件。接着继续填满40M 的数组，然后继续重复之前的步骤。这个思路主要是批量读写文件占优，并且利用内存的数据排序，时间较快。
